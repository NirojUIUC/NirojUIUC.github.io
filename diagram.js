...
console.log(all_cntry_list)
//range = company_data.map( (d,i) => +d.size.split('-')[1] )
//max_range=d3.max(range)

total_comp = all_cntry_list.map( (d,i) => +d.tot)
max_total=d3.max(total_comp)

//all_cntry_names=all_files.map((d,i) => d.file_name.split("_")[0].toUpperCase())
//console.log(all_cntry_list)
